...
- **Import readiness** — four-valued projection over the Import's Shot Requests and rows, never persisted (ruled 2026-09-01, closing the vacuous-readiness question): `READY` — at least one Shot Request exists and every Shot Request is `READY` or `CLOSED`; `PARTIAL` — Shot Requests exist and at least one is elsewhere in the lifecycle; `NOT_STARTED` — zero Shot Requests but workable ideas exist (e.g. everything deferred at confirm); `NO_REQUESTS` — zero Shot Requests and no row ever had a workable idea (INVALID rows never count as workable). Blank Shot Ideas (NO_REQUEST) and deferred (unchecked) rows never block; `READY` + deferred rows is still `READY`; readiness is never vacuously `READY`.
- **Export** — regenerated CSV, downloadable anytime, per Import: every original row, original ordering, original + unknown columns semantically preserved, our columns appended (Shot Status, Approved Count, Approved Image 1..N, Last Reviewed At). Must feel like "their sheet came back with answers," never a report from our data model. Approved Image column count = `max(3, highest requiredApprovals among the Import's Requests)` — schema derives from targets, not current approvals; unfilled slots blank. Filename carries original basename + readiness token (`ready` / `partial` / `not-started` / `no-requests`) + export date (`september-drop-ready-2026-09-01.csv`); no metadata rows in the CSV.
