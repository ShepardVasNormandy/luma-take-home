...
1. **submit**: attempts in SUBMITTING → set POSTING → POST to Luma → save provider id + state QUEUED. 429 → release row, honor Retry-After via next_retry-style backoff. Boot recovery: POSTING w/o provider id older than 2 min → UNKNOWN (manual retry only). **Before implementing, verify against current Luma docs (Context7/live docs, not memory) whether create-generation supports a provider-side idempotency/request key: if yes, send it and recovery can safely re-POST with the same key; if no, the POSTING→UNKNOWN design above stands.**
