...
- **Product decision, observed live and later ruled (2026-09-01):** an import whose eligible rows were all deferred showed "Import ready" and exported a `-ready-` filename — vacuously true under the original definition (every Shot Request is READY or CLOSED — and there were none), but misleading: nothing styled existed yet. Readiness is now a four-valued projection: `READY` requires at least one Shot Request; zero-request imports split into `NOT_STARTED` (workable ideas existed, all deferred) and `NO_REQUESTS` (no row ever had a workable idea). DEFERRED and NO_REQUEST rows stay non-blocking, and the export filename carries the matching token.
