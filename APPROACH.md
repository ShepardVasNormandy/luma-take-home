...
- **Open product decision, observed live and deliberately unchanged:** an import whose eligible rows were all deferred shows "Import ready" and exports a `-ready-` filename. This is vacuously true under the locked definition (every Shot Request is READY or CLOSED — and there are none), but the wording can mislead: nothing styled exists yet. Once real work starts the label correctly flips to partial. Whether "ready" should require at least one approved shot is a product call, parked on purpose.
